...
def main():
    ...
    r = int(input('Existem quantas replicatas do ponto central? '))

    alfa = 0.05
    mult = 2

    combvariaveis = list()
    for aux1 in range(2, k + 1):
        combinacoes = list()
        for aux2 in range(1, k + 1):
    ...
    media_repeticao = np.sum(maty[mat_sqe.size - r:mat_sqe.size]) / r
    #print(media_repeticao)
    for i in range(mat_sqe.size - r, mat_sqe.size):
    ...
    num_gl = np.array([matbetas.size - 1, 2 ** k + r - matbetas.size, 2 ** k + 1 - matbetas.size, r - 1])
